...
#define SETTINGS_MODE_ID \
  "ID"  ///< String that displayed in settings_mode_t = ID
        ...
        if (btn_1_set_mode_counter == 0) {
          ...
          switch (btn_1_settings_mode) {
            ...
            case ID:
              ...
              setting_symbols(temp_str, &drawing_data, ADDR_ID_LIMIT, NULL, 0);

              selected_id = id;
              while (btn_1_settings_mode == ID &&
                     btn_2_set_value_counter == 1 &&
                     btn_1_set_mode_counter == 2) {
                draw_string_on_matrix(temp_str);
              }
